...
<div class="menu_bg">
    <div class="menu">
                            ...
                            <div class="zj">
                                <div class="zj_l">
                                    <c:forEach items="${temp.productCategoryVoList}" var="vo">
                                        <div class="zj_l_c">
                                            <h2>
                                                <a href="${ctx}/queryProductList.action?category=${vo.productCategory.id}&level=2">${vo.productCategory.name}</a>
                                            </h2>
                                            <c:forEach items="${vo.productCategoryVoList}" var="vo2">
                                                <a href="${ctx}/queryProductList.action?category=${vo2.productCategory.id}&level=3">${vo2.productCategory.name}</a> |
                                            </c:forEach>
                                        </div>
                                    </c:forEach>
                                </div>
        ...
        <ul class="menu_r">
            ...
            <c:forEach items="${productCategoryVoList}" var="temp">
                <li><a href="${ctx}/queryProductList.action&level=1&category=${temp.productCategory.id}">${temp.productCategory.name}</a></li>
            </c:forEach>
        </ul>
...
<div class="i_bg bg_color">
    <div class="i_ban_bg">
        ...
        <div class="inews">
            <div class="news_t">
                <span class="fr"><a href="${ctx}/sys/queryNewsList.action">更多 ></a></span>新闻资讯
            </div>
            <ul>
                <c:forEach items="${news}" var="temp">
                    <li><span>[ 公告 ]</span>
                        <a href="${ctx}/sys/queryNewsDeatil.action?id=${temp.id}">${temp.title}</a>
                    </li>
                </c:forEach>
            </ul>
            <div class="charge_t">
